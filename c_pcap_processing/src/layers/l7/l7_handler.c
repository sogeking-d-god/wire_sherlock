 ...
 */
static boolean_e confirm_http_via_hybrid_gate(const flow_key_t *key,
                                              http_kind_e kind,
                                              ...
                                              size_t payload_len)
{
    ...
    else if (http_detector_is_likely_port(key->src_port) == TRUE ||
             http_detector_is_likely_port(key->dst_port) == TRUE)
    {
        ret_val = TRUE;
    }
    else
 ...
 */
static void log_match_chain(const http_attack_match_t *matches_head,
                            const char *flow_buf,
                            uint8_t dev_idx)
{
    ...
    {
        fprintf(stderr,
                "[L7][ATTACK] %s dev=%u sig=%s class=%d offset=%zu len=%zu matched='%s'\n",
                flow_buf,
                (unsigned)dev_idx,
 ...
 */
static void scan_completed_message(const uint8_t *message_bytes,
                                   size_t message_len,
                                   const char *flow_buf,
                                   uint8_t dev_idx,
                                   uint32_t *out_match_count)
{
    ...
    sig_ret = http_signatures_scan_buffer(message_bytes,
                                          message_len,
                                          &matches_head,
                                          &local_match_count);
 ...
 *
 * Wraps the pread arithmetic in one place. The caller passes a scratch buffer
 * of the right size.
 *
 ...
 */
static ssize_t read_message_payload(int pcap_fd,
                                    ...
                                    size_t scratch_cap)
{
    ...
    file_off = (off_t)msg->packet_start_pointer
             + (off_t)PCAP_PACKET_HEADER_SIZE
             + (off_t)msg->data_ptr;
 ...
 * the next (now-IDLE) state. This is how HTTP pipelining is handled.
 *
 ...
 */
static void drain_completed_messages(session_node_t *session,
                                     ...
                                     uint32_t *out_match_count)
{
    ...
    while (draining == TRUE)
    {
        ...
        completed_len = 0;
        feed_result = http_reassembler_feed_payload(reasm_session,
                                                    dev_idx,
                                                    feed_bytes,
                                                    feed_bytes_len,
                                                    fin_seen,
        ...
        {
            fprintf(stderr,
                    "[L7][COMPLETE] %s dev=%u msg_bytes=%zu\n",
            ...
            else
            {
                ...
                if (http_detector_check_prefix(direction->message_buffer,
                                               leftover_count,
                                               &detected_kind,
                                               &detected_verb,
                                               &detected_proto) == TRUE &&
                    detected_proto == L7_PROTO_HTTP1)
                {
                    confirmed = confirm_http_via_hybrid_gate(key,
                                                             detected_kind,
                                                             direction->message_buffer,
                                                             leftover_count);
                    if (confirmed == TRUE)
                    {
                        ...
                        (*out_hits)++;
                        fprintf(stderr,
                                "[L7][HTTP] %s dev=%u (pipelined) leftover=%zu kind=%s verb=%s\n",
        ...
        {
            fprintf(stderr,
                    "[L7][TRUNCATED] %s dev=%u msg_bytes=%zu (hit 16KB cap)\n",
                    flow_buf, (unsigned)dev_idx, completed_len);
            scan_completed_message(completed_bytes, completed_len, flow_buf, dev_idx, out_match_count);
            http_reassembler_reset_direction(reasm_session, dev_idx);
            draining = FALSE;
 ...
 */
static void feed_message_through_reassembler(int pcap_fd,
                                             const flow_key_t *key,
                                             ...
                                             uint32_t *out_hits,
                                             uint32_t *out_match_count,
                                             l7_protocol_e *out_session_proto)
{
    ssize_t got;
    http_kind_e detected_kind;
    const char *detected_verb;
    ...
    if (processing_active == TRUE)
    {
        ...
        // If this direction is still IDLE, the next payload must look like HTTP. We
        // run the prefix detector + hybrid gate now so non-HTTP TCP traffic does
        // not start consuming our 16KB buffer.
        if (direction == NULL || direction->current_state == HTTP_REASSEMBLY_STATE_IDLE)
        {
            if (http_detector_check_prefix(payload_buf,
                                           (size_t)got,
                                           &detected_kind,
                                           &detected_verb,
                                           &detected_proto) == TRUE)
            {
                ...
                else if (detected_proto == L7_PROTO_HTTP1)
                {
                    ...
                    if (confirmed == TRUE)
                    {
                        if (reasm_session == NULL)
                        {
                            reasm_session = http_reassembler_session_alloc();
                        ...
                        if (reasm_session != NULL)
                        {
                            ...
                            (*out_hits)++;
                            fprintf(stderr,
                                    "[L7][HTTP] %s dev=%u seq=%u len=%u kind=%s verb=%s\n",
                                    flow_buf,
        ...
        if (direction != NULL &&
            direction->current_state != HTTP_REASSEMBLY_STATE_GIVE_UP)
        {
            fin_seen = (msg->tcp_flags & TH_FIN) ? TRUE : FALSE;
            drain_completed_messages(session,
                                     key,
                                     msg->dev_idx,
                                     payload_buf,
                                     (size_t)got,
 ...
 * @param out_match_count Incremented for each attack match.
 */
static void process_session(int pcap_fd,
                            const flow_key_t *key,
                            session_node_t *session,
                            uint32_t *out_hits,
                            uint32_t *out_match_count)
{
    ...
    while (msg != NULL && session_proto != L7_PROTO_TLS && session_proto != L7_PROTO_HTTP2)
    {
        ...
        {
            feed_message_through_reassembler(pcap_fd, key, session, msg,
                                             payload_buf, flow_buf,
                                             out_hits, out_match_count,
                                             &session_proto);
        }
        msg = msg->next;
    }
